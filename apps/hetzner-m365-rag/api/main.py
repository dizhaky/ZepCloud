...
try:
    ...
    from raganything.retrievers import HybridRetriever
...
class M365SyncRequest(BaseModel):
    source_type: str = Field(..., pattern="^(sharepoint|onedrive|teams|outlook)$")
...
async def lifespan(app: FastAPI):
    ...
    if RAG_ANYTHING_AVAILABLE:
        ...
        logger.warning("⚠️  RAG-Anything not available - running with limited functionality")
...
app = FastAPI(
    title="M365 RAG API",
    description="Multimodal RAG System for Microsoft 365 Documents on Hetzner",
    version="1.0.0",
    lifespan=lifespan
)
...
async def ensure_indices():
    ...
    if not await es_client.indices.exists(index="documents"):
        await es_client.indices.create(index="documents", body=documents_mapping)
    ...
    if not await es_client.indices.exists(index="images"):
    ...
    if not await es_client.indices.exists(index="knowledge_graph"):
        await es_client.indices.create(index="knowledge_graph", body=kg_mapping)
...
async def process_document(file_path: str, doc_id: str, metadata: Dict):
    """Process document with RAG-Anything"""
    try:
        logger.info(f"Processing document: {file_path}")
        
        if not RAG_ANYTHING_AVAILABLE:
            logger.warning("RAG-Anything not available, skipping advanced processing")
            return
        
        # Parse document with RAG-Anything
        parsed_doc = await rag_engine.parse_document(file_path)
        
        # Extract text chunks
        chunks = parsed_doc.get_chunks(
        ...
        logger.error(f"Processing error for {doc_id}: {str(e)}", exc_info=True)
    finally:
        # Cleanup
...
async def sync_m365(sync_request: M365SyncRequest, background_tasks: BackgroundTasks):
